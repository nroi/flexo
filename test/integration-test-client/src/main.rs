...
fn main() {
    ...
    download_large_file_cached_resume();
    println!("download_large_file_cached_resume:         [SUCCESS]");
    flexo_test_download_file_malformed_xattr();
    println!("flexo_test_download_file_malformed_xattr:  [SUCCESS]");
    flexo_test_parallel_downloads_nonblocking(&mut path_generator);
    println!("flexo_test_parallel_downloads_nonblocking: [SUCCESS]");
}
...
fn download_large_file_cached_resume() {
...
fn flexo_test_download_file_malformed_xattr() {
    let get_requests = vec![
        GetRequest {
...
fn flexo_test_parallel_downloads_nonblocking(path_generator: &mut PathGenerator) {
    ...
    thread::spawn(move || {
        ...
        sender1.send(results).unwrap();
    });
    thread::spawn(move || {
        let results = http_get(request_test_2);
        sender2.send(results).unwrap();
    });
