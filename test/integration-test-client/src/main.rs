...
// Regression test for issue #93. A file that is partially cached on the flexo server (50
// of 100 bytes), used to trigger an HTTP range request (resume) to the mirror.
const RESUME_LEAK_TRIGGER_REQUEST_PATH: &str = "/partially-cached-leak";
const RESUME_LEAK_TRIGGER_SIZE: usize = 100;
// A separate, fully-uncached file fetched right after the trigger over the same reused
// connection. It must be downloaded in full from byte 0; if a stale resume offset leaks
// in, the mirror replies 206 and flexo serves a front-truncated file of the wrong size.
const RESUME_LEAK_VICTIM_REQUEST_PATH: &str = "/resume-leak-victim";
const RESUME_LEAK_VICTIM_SIZE: usize = 4096;
// sha256 of 4096 zero bytes (the full, correct content of the victim file).
const RESUME_LEAK_VICTIM_SHA: &str =
...
fn main() {
    ...
    let all_tests: Vec<FlexoTest> = vec![
        ...
        FlexoTest {
            description: "flexo_test_mirror_stalling_no_fallback",
            action: flexo_test_mirror_stalling_no_fallback,
        },
        FlexoTest {
            description: "flexo_test_mirror_stalling_with_fallback",
            action: flexo_test_mirror_stalling_with_fallback,
        },
        FlexoTest {
        ...
        FlexoTest {
            description: "flexo_test_resume_offset_not_leaked_across_channel_reuse",
            action: flexo_test_resume_offset_not_leaked_across_channel_reuse,
        },
        FlexoTest {
...
fn flexo_test_mirror_stalling_no_fallback(path_generator: &mut PathGenerator, testcase: &'static str) {
    use std::io::{Read, Write};
    use std::net::TcpStream;
    use std::time::Instant;

    let host = "flexo-server-mirror-stalling-no-fallback";
    let path = path_generator.generate();
    let request = format!("GET {} HTTP/1.1\r\nHost: {}\r\n{}", path, host, HEADER_SEPARATOR_STR);

    let mut stream = TcpStream::connect((host, DEFAULT_PORT)).unwrap();
    ...
    loop {
        match stream.read(&mut buf) {
            // EOF: flexo gave up on the stalled download and closed the connection (fixed behaviour).
            Ok(0) => {
                info!("{}: flexo closed the stalled connection after {:?}", testcase, started.elapsed());
                break;
            }
            // Response header (and possibly a partial body) — keep reading until EOF.
            Ok(_) => continue,
            // A read timeout (or other error) means flexo never closed the connection: the serving
            // loop is hanging/spinning on the stalled download (regression of issue #111).
            Err(e) => panic!(
                "{}: flexo did not terminate the stalled transfer within {:?} (read error: {:?}). \
                 The serving loop is hanging or spinning on a stalled download (issue #111).",
                testcase, read_timeout, e,
            ),
        }
    }
}
...
fn flexo_test_mirror_stalling_with_fallback(path_generator: &mut PathGenerator, testcase: &'static str) {
    let get_requests = vec![
        GetRequest {
            path: path_generator.generate(),
            client_header: AutoGenerated,
    ...
    let request_test = GetRequestTest {
        conn_addr: ConnAddr {
            host: "flexo-server-mirror-stalling-with-fallback".to_owned(),
            port: DEFAULT_PORT,
        },
        get_requests,
...
/// Regression test for https://github.com/nroi/flexo/issues/93.
///
/// Flexo reuses server-to-server connections (the curl easy handle) across downloads.
/// `CURLOPT_RESUME_FROM` is sticky on that handle, so if one download sets a resume
/// offset and the next download on the same reused handle does not reset it, flexo issues
/// a bogus `Range: bytes=X-` request for a file that should be fetched from byte 0. The
/// mirror then replies 206 and flexo caches/serves only the tail of the file — exactly the
/// front-truncated, PGP-invalid packages reported in issue #93.
///
/// Both requests are sent over a single client connection so that flexo reuses one
/// server-to-server channel for them (same mechanism exercised by
/// `flexo_test_persistent_connections_s2s`):
///   1. `/partially-cached-leak` is 50-of-100 bytes cached, so fetching it makes flexo
///      resume from byte 50 against the mirror, leaving resume_from=50 on the handle.
///   2. `/resume-leak-victim` is fully uncached and must be downloaded in full. If the
///      stale offset leaks in, the victim comes back truncated (wrong size and sha).
fn flexo_test_resume_offset_not_leaked_across_channel_reuse(
    _path_generator: &mut PathGenerator,
    testcase: &'static str,
) {
    let request_test = GetRequestTest {
        conn_addr: ConnAddr {
        ...
        get_requests: vec![
            // First request: triggers a resume (range request) against the mirror.
            GetRequest {
                path: RESUME_LEAK_TRIGGER_REQUEST_PATH.to_owned(),
                client_header: AutoGenerated,
            },
            // Second request, on the same reused connection: must be downloaded in full.
            GetRequest {
                path: RESUME_LEAK_VICTIM_REQUEST_PATH.to_owned(),
                client_header: AutoGenerated,
            },
        ],
        timeout: Some(Duration::from_millis(60_000)),
    ...
    // Sanity check: the trigger itself must be served completely (50 cached + 50 resumed).
    let trigger = results.get(0).unwrap();
    assert_eq!(trigger.header_result.status_code, 200);
    assert_eq!(trigger.payload_result.as_ref().unwrap().size, RESUME_LEAK_TRIGGER_SIZE);

    // The actual regression check: the victim must come back complete and intact, not
    // truncated by a leftover resume offset from the previous download.
    let victim = results.get(1).unwrap();
    assert_eq!(victim.header_result.status_code, 200);
    assert_eq!(victim.payload_result.as_ref().unwrap().size, RESUME_LEAK_VICTIM_SIZE);
    let sha: String = hex::encode(victim.payload_result.as_ref().unwrap().clone().sha);
    assert_eq!(sha, RESUME_LEAK_VICTIM_SHA);
}
